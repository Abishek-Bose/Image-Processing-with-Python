...
class T_CNN(object):
  ...
  def train(self, config):
    ...
    for id in range(0,1):
        ...
        imsave_lable(final, image_path)   #saving the image
  ...
  def model(self):
    ...
    with tf.variable_scope("model_h") as scope:
        ...
        image_conv2 = tf.nn.relu(conv2d(image_conv1, 16, k_h=3, k_w=3, d_h=1, d_w=1,name="conv2d_dehaze2")) #convoluting the image thrice
        image_conv3 = tf.nn.relu(conv2d(image_conv2, 16, k_h=3, k_w=3, d_h=1, d_w=1,name="conv2d_dehaze3"))
        dehaze_concat1 = tf.concat(axis = 3, values = [image_conv1,image_conv2,image_conv3,self.images])            # concatinating the convoluted images
        ...
        dehaze_concat2 = tf.concat(axis = 3, values = [dehaze_concat1,image_conv4,image_conv5,image_conv6])          #iterating the entire process thrice
        ...
        dehaze_concat3 = tf.concat(axis = 3, values = [dehaze_concat2,image_conv7,image_conv8,image_conv9]) # returning the output 
