from model import T_CNN                                                                                       # importing the T_CNN model
from utils import *
import numpy as np
import tensorflow.compat.v1 as tf

import pprint   # console display
import os
...
def main(_):
  ...
  if not os.path.exists(FLAGS.checkpoint_dir):   # creating the directory in case path doesnt exists....
    os.makedirs(FLAGS.checkpoint_dir)
  if not os.path.exists(FLAGS.sample_dir):
    os.makedirs(FLAGS.sample_dir)
  filenames = os.listdir('test_real')                   # taking the input images
  data_dir = os.path.join(os.getcwd(), 'test_real')
  data = glob.glob(os.path.join(data_dir, "*.bmp"))
  test_data_list = data + glob.glob(os.path.join(data_dir, "*.png")) #allocating path for output images

  for ide in range(0,len(test_data_list)):
    image_test =  get_image(test_data_list[ide],is_grayscale=False)  # converting images to non greyscale
    ...
    with tf.Session() as sess:
      # with tf.device('/cpu:0'):
        ...
        srcnn.train(FLAGS) # training with the data & generating output for the test images
        sess.close()
